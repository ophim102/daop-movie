...
create policy "Admin only" on public.ad_banners for all using ((auth.jwt() ->> 'role') = 'admin');
create policy "Admin only" on public.ad_preroll for all using ((auth.jwt() ->> 'role') = 'admin');
create policy "Admin only" on public.homepage_sections for all using ((auth.jwt() ->> 'role') = 'admin');
create policy "Admin only" on public.server_sources for all using ((auth.jwt() ->> 'role') = 'admin');
create policy "Admin only" on public.site_settings for all using ((auth.jwt() ->> 'role') = 'admin');
create policy "Admin only" on public.static_pages for all using ((auth.jwt() ->> 'role') = 'admin');
create policy "Admin only" on public.donate_settings for all using ((auth.jwt() ->> 'role') = 'admin');
create policy "Admin only" on public.player_settings for all using ((auth.jwt() ->> 'role') = 'admin');
create policy "Admin only" on public.audit_logs for all using ((auth.jwt() ->> 'role') = 'admin');
