...
Tài liệu này mô tả từng bước đưa dự án lên môi trường thật: website trên Cloudflare Pages, Admin trên Vercel, dữ liệu từ Supabase, ảnh qua GitHub + jsDelivr, hệ thống comment nội bộ.
...
| **Cloudflare** | Pages (website) | Có (Pages) |
...
**Ảnh / CDN (mặc định):** Secret `IMAGE_CDN_BASE` (vd. `https://cdn.jsdelivr.net/gh/ophim102/cm114@main/public`), `IMAGES_REPO`, tùy chọn `IMAGES_TOKEN` / `IMAGES_BRANCH` — trùng cấu hình trên Vercel. Thêm `OPHIM_BASE_URL` nếu endpoint OPhim khác mặc định.
...
**Danh sách gom mọi thứ cần tạo trên Cloudflare (Pages, token, comment):** [cloudflare/README.md](./cloudflare/README.md).
...
4. Chọn một trong hai:
   ...
   - **Custom token:** **Create Custom Token** → đặt tên (vd: `daop-pages-deploy`), phần **Permissions**:
     - **Tối thiểu cần có:** **Account** → **Cloudflare Pages** → **Edit** (để deploy/upload thư mục `public/`)
     - Không cần quyền object storage chuyên dụng nếu bạn dùng ảnh qua **GitHub + jsDelivr**.
...
- **Ảnh:** GitHub repo + jsDelivr (`IMAGE_CDN_BASE`, `IMAGES_*`); build/workflow sync `public/thumbs` & `posters`.
...
Nếu gặp lỗi cụ thể (build, deploy, đăng nhập, sync user…), có thể đối chiếu thêm với `docs/` tương ứng (supabase, cloudflare-pages, vercel, github-actions, comments).
